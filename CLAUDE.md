...
- 中国PMI采购经理人指数数据（月度）
- 指数日线行情数据（日度，沪深300、中证500、上证50、上证综指等）
...
所有数据统一使用 `TIMESTAMPTZ` (带时区的timestamp) 存储，时区统一为 `Asia/Shanghai` (UTC+8)：
...
**重要说明**:
1. **时区统一**: 所有数据统一使用 `Asia/Shanghai` 时区存储，即使未来支持港股/美股数据也将遵循此规范
2. **收盘时间标记**: 日频及以上数据统一标记为对应市场的收盘时间（中国大陆 15:00，港股 16:00）
3. **查询返回**: SDK 返回的 DataFrame 中 `time` 列为 `datetime64[ns, Asia/Shanghai]` 类型
4. **日期参数**: SDK 查询方法接受 `YYYY-MM-DD` 字符串格式（如 `'2024-01-01'`），内部自动转换为带时区的时间戳
...
# 指数日线行情数据查询
# 支持的指数代码来自 SUPPORTED_INDEX_CODES，可通过 ts_code 或日期范围查询
index_daily = await fdh.get_index_daily_async(ts_code='000300.SH', start_date='2024-01-01', end_date='2024-12-31')
print(f"沪深300指数日线: {len(index_daily)} 条记录")
print(index_daily[['trade_date', 'close', 'pct_chg', 'vol']].head())
...
- 支持的指数代码来自项目内 `SUPPORTED_INDEX_CODES`
- 返回字段: `ts_code`, `trade_date`, `close`, `open`, `high`, `low`, `pre_close`, `change`, `pct_chg`, `vol`, `amount`
- Tushare API 单次限制 8000 条记录，系统自动分页补齐更早历史数据
- CLI 不支持 `--trade-date` 的全指数单日批量模式，请使用 `--symbols` 或 `--start-date/--end-date`
...
# 更新指数日线行情数据（项目支持指数）
fdh-cli update --dataset index_daily               # 智能增量更新（无数据时等同于全量）
fdh-cli update --dataset index_daily --force       # 强制全量更新
fdh-cli update --dataset index_daily --start-date 2024-01-01 --end-date 2024-12-31  # 指定日期范围
fdh-cli update --dataset index_daily --symbols 000300.SH  # 指定指数代码
fdh-cli update --dataset index_daily --trade-date 2024-11-27  # 获取指定交易日所有有效指数数据
