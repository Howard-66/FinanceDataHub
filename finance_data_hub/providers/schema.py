...
FinaIndicatorSchema = DataFrameSchema(
    ...
    columns=[
        ...
        ColumnSchema("invturn_days", "float64", True, "存货周转天数"),
        ColumnSchema("arturn_days", "float64", True, "应收账款周转天数"),
        ColumnSchema("inv_turn", "float64", True, "存货周转率"),
        ...
        ColumnSchema("valuechange_income", "float64", True, "价值变动净收益"),
        ColumnSchema("interst_income", "float64", True, "利息费用"),
        ColumnSchema("daa", "float64", True, "折旧与摊销"),
        ...
        ColumnSchema("retainedps", "float64", True, "每股留存收益"),
        ColumnSchema("cfps", "float64", True, "每股现金流量净额"),
        ColumnSchema("ebit_ps", "float64", True, "每股息税前利润"),
        ColumnSchema("fcff_ps", "float64", True, "每股企业自由现金流量"),
        ColumnSchema("fcfe_ps", "float64", True, "每股股东自由现金流量"),
        ColumnSchema("netprofit_margin", "float64", True, "销售净利率"),
        ColumnSchema("grossprofit_margin", "float64", True, "销售毛利率"),
        ColumnSchema("cogs_of_sales", "float64", True, "销售成本率"),
        ColumnSchema("expense_of_sales", "float64", True, "销售期间费用率"),
        ColumnSchema("profit_to_gr", "float64", True, "净利润/营业总收入"),
        ColumnSchema("saleexp_to_gr", "float64", True, "销售费用/营业总收入"),
        ColumnSchema("adminexp_of_gr", "float64", True, "管理费用/营业总收入"),
        ColumnSchema("finaexp_of_gr", "float64", True, "财务费用/营业总收入"),
        ColumnSchema("impai_ttm", "float64", True, "资产减值损失/营业总收入"),
        ColumnSchema("gc_of_gr", "float64", True, "营业总成本/营业总收入"),
        ColumnSchema("op_of_gr", "float64", True, "营业利润/营业总收入"),
        ColumnSchema("ebit_of_gr", "float64", True, "息税前利润/营业总收入"),
        ...
        ColumnSchema("npta", "float64", True, "总资产净利润"),
        ColumnSchema("roic", "float64", True, "投入资本回报率"),
        ColumnSchema("roe_yearly", "float64", True, "年化净资产收益率"),
        ColumnSchema("roa2_yearly", "float64", True, "年化总资产报酬率"),
        ColumnSchema("roe_avg", "float64", True, "平均净资产收益率(增发条件)"),
        ColumnSchema("opincome_of_ebt", "float64", True, "经营活动净收益/利润总额"),
        ColumnSchema("investincome_of_ebt", "float64", True, "价值变动净收益/利润总额"),
        ColumnSchema("n_op_profit_of_ebt", "float64", True, "营业外收支净额/利润总额"),
        ColumnSchema("tax_to_ebt", "float64", True, "所得税/利润总额"),
        ColumnSchema("dtprofit_to_profit", "float64", True, "扣除非经常损益后的净利润/净利润"),
        ColumnSchema("salescash_to_or", "float64", True, "销售商品提供劳务收到的现金/营业收入"),
        ColumnSchema("ocf_to_or", "float64", True, "经营活动产生的现金流量净额/营业收入"),
        ColumnSchema("ocf_to_opincome", "float64", True, "经营活动产生的现金流量净额/经营活动净收益"),
        ColumnSchema("capitalized_to_da", "float64", True, "资本支出/折旧和摊销"),
        ColumnSchema("debt_to_assets", "float64", True, "资产负债率"),
        ColumnSchema("assets_to_eqt", "float64", True, "权益乘数"),
        ColumnSchema("dp_assets_to_eqt", "float64", True, "权益乘数(杜邦分析)"),
        ...
        ColumnSchema("ocf_to_shortdebt", "float64", True, "经营活动产生的现金流量净额/流动负债"),
        ...
        ColumnSchema("tangasset_to_intdebt", "float64", True, "有形资产/带息债务"),
        ColumnSchema("tangibleasset_to_netdebt", "float64", True, "有形资产/净债务"),
        ColumnSchema("ocf_to_debt", "float64", True, "经营活动产生的现金流量净额/负债合计"),
        ColumnSchema("ocf_to_interestdebt", "float64", True, "经营活动产生的现金流量净额/带息债务"),
        ColumnSchema("ocf_to_netdebt", "float64", True, "经营活动产生的现金流量净额/净债务"),
        ColumnSchema("ebit_to_interest", "float64", True, "已获利息倍数(EBIT/利息费用)"),
        ColumnSchema("longdebt_to_workingcapital", "float64", True, "长期债务与营运资金比率"),
        ColumnSchema("ebitda_to_debt", "float64", True, "息税折旧摊销前利润/负债合计"),
        ColumnSchema("turn_days", "float64", True, "营业周期"),
        ColumnSchema("roa_yearly", "float64", True, "年化总资产净利率"),
        ColumnSchema("roa_dp", "float64", True, "总资产净利率(杜邦分析)"),
        ...
        ColumnSchema("nop_to_ebt", "float64", True, "非营业利润/利润总额"),
        ColumnSchema("ocf_to_profit", "float64", True, "经营活动产生的现金流量净额/营业利润"),
        ColumnSchema("cash_to_liqdebt", "float64", True, "货币资金/流动负债"),
        ColumnSchema("cash_to_liqdebt_withinterest", "float64", True, "货币资金/带息流动负债"),
        ColumnSchema("op_to_liqdebt", "float64", True, "营业利润/流动负债"),
        ColumnSchema("op_to_debt", "float64", True, "营业利润/负债合计"),
        ColumnSchema("roic_yearly", "float64", True, "年化投入资本回报率"),
        ColumnSchema("total_fa_trun", "float64", True, "固定资产合计周转率"),
        ColumnSchema("profit_to_op", "float64", True, "利润总额/营业收入"),
        ColumnSchema("q_opincome", "float64", True, "经营活动单季度净收益"),
        ColumnSchema("q_investincome", "float64", True, "价值变动单季度净收益"),
        ...
        ColumnSchema("q_exp_to_sales", "float64", True, "销售期间费用率(单季度)"),
        ColumnSchema("q_profit_to_gr", "float64", True, "净利润/营业总收入(单季度)"),
        ColumnSchema("q_saleexp_to_gr", "float64", True, "销售费用/营业总收入(单季度)"),
        ColumnSchema("q_adminexp_to_gr", "float64", True, "管理费用/营业总收入(单季度)"),
        ColumnSchema("q_finaexp_to_gr", "float64", True, "财务费用/营业总收入(单季度)"),
        ColumnSchema("q_impair_to_gr_ttm", "float64", True, "资产减值损失/营业总收入(单季度)"),
        ColumnSchema("q_gc_to_gr", "float64", True, "营业总成本/营业总收入(单季度)"),
        ColumnSchema("q_op_to_gr", "float64", True, "营业利润/营业总收入(单季度)"),
        ColumnSchema("q_roe", "float64", True, "净资产收益率(单季度)"),
        ColumnSchema("q_dt_roe", "float64", True, "净资产单季度收益率(扣除非经常损益)"),
        ColumnSchema("q_npta", "float64", True, "总资产净利润(单季度)"),
        ColumnSchema("q_opincome_to_ebt", "float64", True, "经营活动净收益/利润总额(单季度)"),
        ColumnSchema("q_investincome_to_ebt", "float64", True, "价值变动净收益/利润总额(单季度)"),
        ColumnSchema("q_dtprofit_to_profit", "float64", True, "扣除非经常损益后的净利润/净利润(单季度)"),
        ...
        ColumnSchema("q_ocf_to_or", "float64", True, "经营活动产生的现金流量净额/经营活动净收益(单季度)"),
        ...
        ColumnSchema("q_gr_qoq", "float64", True, "营业总收入环比增长率(%)(单季度)"),
        ColumnSchema("q_sales_yoy", "float64", True, "营业收入同比增长率(%)(单季度)"),
        ColumnSchema("q_sales_qoq", "float64", True, "营业收入环比增长率(%)(单季度)"),
        ...
        ColumnSchema("update_flag", "object", True, "更新标识"),
    ],
)
