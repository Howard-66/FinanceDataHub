...
数据来源：
- fina_indicator: ROA, 毛利率, 资产周转率, current_ratio
- balancesheet: 总资产, 总负债, 流动资产, 流动负债, 股本变化
- cashflow: 经营现金流
- income: 净利润
"""
...
class FScoreCalculator:
    ...
    def _calc_single_stock(
        ...
        # F_ΔROA: ROA TTM 同比增长
        result["roa_ttm"] = self._calc_roa_ttm(result)
        ...
        if "f_score_gross_margin" in exemptions:
            ...
            result["f_delta_margin"] = self._calc_yoy_improvement(result, "grossprofit_margin")
        else:
            result["f_delta_margin"] = 0
        
        # F_ΔTURN: 资产周转率上升
        if "assets_turn" in result.columns:
            result["f_delta_turn"] = self._calc_yoy_improvement(result, "assets_turn")
        ...
        # 5年平均ROE（使用年报数据）
        result["roe_5y_avg"] = self._calc_roe_5y_avg(result, "roe")
    ...
    def _calc_roa_ttm(
        ...
        累计值转 TTM 公式:
            Q4(年报): ROA_TTM = ROA_本期
            Q1-Q3:   ROA_TTM = ROA_本期累计 + (ROA_上年年报 - ROA_上年同期累计)
        ...
        Returns:
            TTM ROA Series
        """
        if "roa" not in df.columns or "end_date_time" not in df.columns:
        ...
        roa_values = df["roa"].astype(float)
        
        # 构建快速查找字典: (year, month) -> roa
        lookup = {}
        for idx, (y, m, roa_val) in enumerate(zip(years, months, roa_values)):
            if pd.notna(roa_val):
                lookup[(int(y), int(m))] = roa_val
        
        for i, (idx, row) in enumerate(df.iterrows()):
            cur_date = end_dates.iloc[i]
            cur_month = int(cur_date.month)
            cur_year = int(cur_date.year)
            cur_roa = roa_values.iloc[i]
            ...
            if cur_month == 12:
                ...
                if prev_annual is not None and prev_same_q is not None:
                    result.iloc[i] = cur_roa + (prev_annual - prev_same_q)
    ...
    def _calc_roe_5y_avg(
        ...
        """
        计算5年平均ROE（参考实现：每年取最后一条记录，计算最近5年平均）
        
        对于每一行，计算截至该行日期的5年平均ROE
        """
        if roe_col not in df.columns and "roe_yearly" not in df.columns:
            return pd.Series([np.nan] * len(df), index=df.index)
        
        # 优先使用 roe_yearly，如果没有则使用 roe
        actual_col = "roe_yearly" if "roe_yearly" in df.columns else roe_col
