...
CREATE TABLE IF NOT EXISTS processed_daily_qfq (
    ...
    -- 量能方向指标
    nda_value SMALLINT,                     -- 净派发/吸筹方向值
    volume_confirmed BOOLEAN,               -- 量能方向是否确认
...
CREATE TABLE IF NOT EXISTS processed_fundamental_quality (
    ...
    npm_ttm DECIMAL(20,4),                     -- 净利率 TTM (4期均值)
    gpm_ttm_12q_std DECIMAL(20,4),             -- 毛利率 TTM 12季滚动标准差
    gpm_ttm_12q_delta DECIMAL(20,4),           -- 毛利率 TTM 相对12季前变化
    npm_ttm_12q_std DECIMAL(20,4),             -- 净利率 TTM 12季滚动标准差
    cfo_to_ni_ttm DECIMAL(20,4),               -- 经营现金流 TTM / 净利润 TTM
    buffett_gpm_flag SMALLINT,                 -- 巴菲特毛利率护城河标记
    buffett_npm_stable_flag SMALLINT,          -- 巴菲特净利率稳定标记
    buffett_roa_flag SMALLINT,                 -- 巴菲特ROA标记
    buffett_cashflow_flag SMALLINT,            -- 巴菲特现金流标记
...
CREATE TABLE IF NOT EXISTS processed_cn_macro_cycle_phase (
    time TIMESTAMPTZ NOT NULL,              -- 生效月份（月末 15:00 Asia/Shanghai）
    observation_time TIMESTAMPTZ NOT NULL,  -- 观测月份（月末 15:00 Asia/Shanghai）

    -- 宏观指标
    m2_yoy DECIMAL(10,4),                   -- M2同比
    gdp_yoy DECIMAL(10,4),                  -- GDP同比（季度数据向后对齐到月度）
    ppi_yoy DECIMAL(10,4),                  -- PPI同比
    pmi DECIMAL(10,4),                      -- PMI（优先制造业 PMI）
    credit_impulse DECIMAL(10,4),           -- 信用脉冲 = M2同比 - GDP同比

    -- 周期阶段
    raw_phase VARCHAR(20) NOT NULL,         -- 原始阶段
    stable_phase VARCHAR(20) NOT NULL,      -- 平滑阶段（2个月确认）
...
COMMENT ON COLUMN processed_cn_macro_cycle_phase.time IS '信号生效月份（月末 15:00 Asia/Shanghai），整体滞后一个月';
COMMENT ON COLUMN processed_cn_macro_cycle_phase.observation_time IS '原始观测月份（月末 15:00 Asia/Shanghai）';
COMMENT ON COLUMN processed_cn_macro_cycle_phase.credit_impulse IS '信用脉冲 = M2同比 - GDP同比';
COMMENT ON COLUMN processed_cn_macro_cycle_phase.raw_phase IS '按 ValueInvesting 原始算法得到的中国宏观周期阶段';
COMMENT ON COLUMN processed_cn_macro_cycle_phase.stable_phase IS '抗抖动后的中国宏观周期阶段，新阶段连续出现2个月才切换';
...
CREATE TABLE IF NOT EXISTS processed_cn_macro_cycle_industry (
    time TIMESTAMPTZ NOT NULL,                  -- 生效月份
    observation_time TIMESTAMPTZ NOT NULL,      -- 观测月份

    -- 行业映射（来自 industry_config.json + sw_industry_member）
    l1_code VARCHAR(20),
    l1_name VARCHAR(100),
    l2_code VARCHAR(20),
    l2_name VARCHAR(100),
    ...
    -- 配置信息
    config_macro_cycle VARCHAR(20),             -- 行业适配的宏观周期
    core_indicator VARCHAR(10),                 -- 该行业核心估值指标
    ref_indicator VARCHAR(10),                  -- 该行业参考估值指标
    logic TEXT,                                 -- 行业逻辑说明
    fscore_exemptions JSONB,                    -- F-Score 豁免规则
    is_present_in_sw_member BOOLEAN DEFAULT FALSE, -- 当前是否能在 sw_industry_member 中映射到编码
...
COMMENT ON COLUMN processed_cn_macro_cycle_industry.config_macro_cycle IS '该三级行业在 industry_config.json 中配置的适配宏观周期';
COMMENT ON COLUMN processed_cn_macro_cycle_industry.matches_raw_phase IS '该行业是否匹配 raw_phase';
COMMENT ON COLUMN processed_cn_macro_cycle_industry.matches_stable_phase IS '该行业是否匹配 stable_phase';
