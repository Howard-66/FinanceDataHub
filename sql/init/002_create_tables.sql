...
CREATE TABLE IF NOT EXISTS etf_index (
    ...
    pub_party_name TEXT, pub_date DATE, base_date DATE, bp NUMERIC(24,8), adj_circle TEXT,
    updated_at TIMESTAMPTZ DEFAULT NOW(), created_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_etf_index_pub_date ON etf_index(pub_date);

CREATE TABLE IF NOT EXISTS fund_daily (
    ts_code VARCHAR(32) NOT NULL, trade_date DATE NOT NULL, open NUMERIC(24,8),
    high NUMERIC(24,8), low NUMERIC(24,8), close NUMERIC(24,8), pre_close NUMERIC(24,8),
    change NUMERIC(24,8), pct_chg NUMERIC(24,8), vol NUMERIC(30,6), amount NUMERIC(30,6),
    updated_at TIMESTAMPTZ DEFAULT NOW(), created_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (ts_code, trade_date)
);
...
CREATE TABLE IF NOT EXISTS etf_share_size (
    ...
    close NUMERIC(24,8), exchange VARCHAR(16), updated_at TIMESTAMPTZ DEFAULT NOW(),
    created_at TIMESTAMPTZ DEFAULT NOW(), PRIMARY KEY (ts_code, trade_date)
);
...
CREATE TABLE IF NOT EXISTS idx_anns (
    ...
    type VARCHAR(128), updated_at TIMESTAMPTZ DEFAULT NOW(), created_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (ann_date, source, title)
);
