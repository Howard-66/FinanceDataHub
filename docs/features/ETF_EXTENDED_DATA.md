...
| `etf_sh_cons` | 3,000 | 从最早 ETF 日期开始逐 SSE 交易日 | 从本地最新交易日开始逐日 |
| `etf_sz_cons` | 3,000 | 从最早 ETF 日期开始逐 SSE 交易日 | 从本地最新交易日开始逐日 |
| `idx_anns` | 1,000 | 从 SSE 交易日历最早日期开始，按自然月窗口 | 从最新公告日回看 7 天，按自然月窗口 |

所有 Provider 请求都会显式传入完整字段列表。返回数量达到接口上限时，自动增加
`offset` 继续读取；若服务端忽略 `offset` 并重复返回同一满页，下载会立即报错，避免
无限循环。每个下载批次会立即 upsert，而不是等全量任务结束后统一写库。
